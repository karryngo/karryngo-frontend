...
export class BeProviderFormGuardGuard implements CanActivate {
    constructor( 
    ...
    canActivate(
        ...
        return new Promise<UrlTree | boolean>((resolve,reject)=>{
            ...
            else if(user.isProvider && user.isAcceptedProvider) return resolve(true)
            else if(user.isProvider && !user.isAcceptedProvider) return resolve(this.router.parseUrl("/carrier/wait-acceptance"))
            resolve(this.router.parseUrl("/carrier/be-carrier"))
            })
        })
    }
