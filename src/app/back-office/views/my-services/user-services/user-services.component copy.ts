...
@Component({
    ...
    template: './user-services.component.html',
...
export class UserServicesComponent implements OnInit {
    ...
    // constructCheckOlderPayment(service) {
    //     if(this.selectedPaymentMethod==MethodPaymentState.mtn || this.selectedPaymentMethod==MethodPaymentState.orange)
    //         return {
    ...
    async check_payment_function(toSend){
        ...
        try {
            ...
            if(rep.status== TransactionState.COMPLETED){
                ...
                //         // this.waitSelectedProvider=false;
                //         this.notification.showNotification('top','center', 'success', 'pe-7s-close-circle', '\<b>Success\</b>\<br>Provider has been successfully notified. Now you can finalize transaction in the chat menu')
                //         setTimeout(() => this.router.navigate(['chat/'+this.selected_service._id]), 600);
                //       })
    ...
    //             product: this.product
    //         };
    //     if(this.selectedPaymentMethod==MethodPaymentState.paygate)
    //         objReq = {
    //             idService: this.selected_service._id,
    //             payment_mode: this.selectedPaymentMethod,
    //             moneyCode: this.moneyCode,
    ...
    //         }
    //         if (rep.payment.pay_token)
    //         {
    //             if (rep.payment.success == false) {
    //                 console.log(rep.payment.error)
    //                 this.modalService.dismissAll();
    ...
    //                 this.notification.showNotification('top','center', 'danger', 'pe-7s-close-circle', '\<b>Payment error \</b>\<br>'+ rep.payment.error + '.\<br> check your phone number and try again Please !');
    //                 return;
    //             }
    //             if (rep.payment.paymentMethod = MethodPaymentState.paygate) {
    //                 this.redirectService.redirectToExternalUrl(rep.payment.payment_detail_url);
    //             }
    //         } else {
    //             this.modalService.dismissAll();
    //             this.notification.showNotification('top','center', 'danger', 'pe-7s-close-circle', '\<b>Payment error \</b>\<br>'+ rep.payment.message);
    //             return;
    //         }
            
            
    //         this.getCurrentService('no_msg');
    //         this.objTransaction = rep.data[0];
    ...
    //         } else if (paymentResponse.payment.paymentMethod === MethodPaymentState.paygate) {
    //             console.log(paymentResponse)
